...
# Temporarily no ASR  ( 2017/4/4 jackie-Liu)
...
for q_dir in query_dir_ls:
    ...
    for idx,query in enumerate(queries,1):
        sep_idx = query.find('|')
        if sep_idx != -1:
            query_sent.append(query[:sep_idx].strip(' '))
            query_map.append(str(idx))
            query_sent.append(query[sep_idx+1:].strip(' ').strip())
            query_map.append(str(idx))
        else:
            query_sent.append(query)
            query_map.append(str(idx))

    with open(os.path.join(q_dir,'queries_id.map'),'w') as query_id_file, \
    ...
    tokpath = os.path.join(q_dir,'queries_sep.toks')
    parentpath = os.path.join(q_dir,'queries_sep_dparents')
