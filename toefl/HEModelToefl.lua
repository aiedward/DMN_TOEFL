local HEModelToefl = torch.class('HierEpiModel.HEModelToefl')

function HEModelToefl:__init(config)
  self.num_choices     = 4
  ...
  self.DMN = HierEpiModel.DMN(dmn_config)
...
function HEModelToefl:new_sim_module()
  local choices = {}
  local similarities = {}
...
function HEModelToefl:train(dataset)
  ...
  for i = 1, dataset.size, self.batch_size do
    ...
    local feval = function(x)
      ...
      for j = 1, batch_size do
          ...
          --forward thru dmn
          local dmn_out = self.DMN:forward(memory,query_vecs)
          ...
          --backward thru dynamic memory network
...
function HEModelToefl:predict(container,sents,choices,query,num_sent,num_answers,verbose)
  --revoke sub module
...
function HEModelToefl:predict_dataset(dataset)
  local predictions = {}
...
function HEModelToefl:print_config()
...
function HEModelToefl:save(path)
    local config = {
    level                   = self.level,
...
function HEModelToefl.load(path)
  local state = torch.load(path)
  local model = HierEpiModel.HEModelToefl.new(state.config)
